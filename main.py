...
def face_confidence(face_distance, face_match=0.6):
...
class FaceRecognition:
    ...
    def run_reco(self):
        ...
        while True:
            ret, frame = video_capture.read()  # Certifique-se de capturar um quadro

            if self.process_current_frame:
                small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)
                rgb_small_frame = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB)

                # Encontre os rostos no quadro atual
